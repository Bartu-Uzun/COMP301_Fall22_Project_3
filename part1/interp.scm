    ...
    (lambda (exp env)
      (cases expression exp
        ...
        (proc-nested-exp (b-var count p-name p-body)
          (proc-val (nested-procedure p-name b-var p-body env count)))
        ...
        (call-nested-exp (rator rand count)
          ...
          (let ((num (value-of count env)))
              (let ((proce (expval->proc (value-of rator env)))
                    (arg (value-of rand env)))
                (cases proc proce
                  (nested-procedure (p-name b-var p-body env count)
                    (apply-procedure (nested-procedure p-name b-var p-body (extend-env 'count num env) count) arg))
                  (else
                   (apply-procedure proce arg))))))
  ...
  (define apply-procedure
    (lambda (proc1 arg)
      (cases proc proc1
        ...
        (nested-procedure (p-name b-var p-body saved-env count)
          (begin
            
            (recursive-displayer p-name (expval->num (apply-env saved-env count)))
            (value-of p-body (extend-env b-var arg saved-env))))
